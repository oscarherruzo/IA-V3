# TABS DE EXCEL: GENERAR, VALIDAR, EXPLICAR, EVALUAR Y CHAT
# EQUIVALENTE AL MÓDULO POWER BI PERO ADAPTADO A EXCEL

import streamlit as st

from modules.excel.core_functions import (
    generate_excel_formula, generate_formulas_recomendadas,
    validate_excel_formula, explain_excel_formula,
    evaluate_excel_formula, chat_con_excel,
    generate_formulas_base, generate_doc_excel,
)
...
def init_excel_state():
    defaults = {
        "xl_historial":           [],
        "xl_chat_history":        [],
        "xl_explain_result":      None,
        "xl_validation_result":   None,
        "xl_eval_result":         None,
        "xl_doc_result":          None,
        "xl_campos_result":       None,
        "xl_recomendados_result": None,
    }
...
def render_tabs_excel(current_schema: dict):
    """RENDERIZA LAS 5 TABS DE EXCEL EN LA COLUMNA CENTRAL"""
...
def _tab_generar(schema: dict):
    ...
    user_query = st.text_area(
        ...
        height=130, label_visibility="collapsed", key="xl_query_input",
    )

    col_btn1, col_btn2, col_hint = st.columns([1, 1, 1])
    with col_btn1:
        run = st.button("Generar fórmula", type="primary", key="xl_btn_generar")
    with col_btn2:
        run_rec = st.button(
            "◈ Campos recomendados", type="secondary", key="xl_btn_recomendados",
    ...
    if run:
        if user_query and schema:
            with st.status("Generando fórmula Excel...", expanded=True) as status:
                st.write("Analizando schema de campos disponibles...")
                st.write("Construyendo fórmula Excel...")
                try:
                    result = generate_excel_formula(schema, user_query)
                    status.update(label="Fórmula generada", state="complete")
                    ss.xl_historial.insert(0, {"query": user_query, "result": result})
    ...
    if run_rec:
        if schema is None or len(schema.get("tables", [])) <= 1:
            ...
            with st.status("Analizando tu schema...", expanded=True) as status:
                ...
                try:
                    recomendados = generate_formulas_recomendadas(schema)
                    ss.xl_recomendados_result = recomendados
    ...
    if ss.xl_recomendados_result:
        ...
        render_resultado(ss.xl_recomendados_result, code_lang="sql")
        if st.button("Limpiar recomendados", type="secondary", key="xl_clear_rec"):
            ss.xl_recomendados_result = None
            st.rerun()
    ...
    st.markdown(
        ...
        'La IA genera campos calculados listos para usar en Excel.</p>',
        unsafe_allow_html=True,
    )
    ...
    fuentes_elegidas = st.multiselect(
        ...
        placeholder="Elige una o varias fuentes...", key="xl_campos_fuentes",
    )

    col_nivel, col_gen = st.columns([1, 1])
    with col_nivel:
        nivel = st.selectbox("Nivel", options=["Basicos", "Avanzados", "Ambos"], key="xl_nivel")
    with col_gen:
        st.markdown("<div style='padding-top:28px;'></div>", unsafe_allow_html=True)
        run_campos = st.button("Generar campos", type="primary", key="xl_btn_campos")
        ...
        if not fuentes_elegidas:
            ...
            with st.status(f"Generando campos para {len(fuentes_elegidas)} fuentes...", expanded=True) as status:
                ...
                try:
                    ...
                    ss.xl_campos_result = {"campos": campos, "fuentes": fuentes_elegidas, "nivel": nivel}
    ...
    if ss.xl_campos_result:
        campos_list = ss.xl_campos_result.get("campos", [])
        if campos_list:
            fuentes_con_campos = {}
            ...
            if st.button("Limpiar campos", type="secondary", key="xl_clear_campos"):
                ss.xl_campos_result = None
                st.rerun()
        else:
    ...
    if ss.xl_historial:
        st.markdown('<p class="card-title" style="margin:16px 0 12px 0;">Historial</p>',
                    unsafe_allow_html=True)
        for i, item in enumerate(ss.xl_historial):
            num    = len(ss.xl_historial) - i
            titulo = item["query"][:70] + ("..." if len(item["query"]) > 70 else "")
            with st.expander(f"#{num} — {titulo}"):
                render_resultado(item["result"], code_lang="sql")
        if st.button("Limpiar historial", type="secondary", key="xl_clear_historial"):
            ss.xl_historial = []
            st.rerun()
...
def _tab_validar(schema: dict):
    ...
    if ss.xl_validation_result:
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown('<p class="card-title" style="margin-bottom:6px;">Código analizado</p>',
                    unsafe_allow_html=True)
        st.code(ss.xl_validation_result["codigo"], language="sql")
        st.markdown('<p class="card-title" style="margin:16px 0 4px 0;">Resultado de la validación</p>',
                    unsafe_allow_html=True)
        render_validation(ss.xl_validation_result)
        if st.button("Limpiar validación", type="secondary", key="xl_clear_validate"):
            ss.xl_validation_result = None
            st.rerun()


    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<p class="card-title">Validador de Expresiones Excel</p>', unsafe_allow_html=True)
    st.markdown(
        '<p class="hint-text">Pega una fórmula de campo calculado y la IA la validará '
        'contra tu schema: detecta campos inexistentes, errores de sintaxis y problemas de lógica.</p>',
        unsafe_allow_html=True,
    )

    expr_to_validate = st.text_area(
        "Pega aquí la fórmula Excel a validar",
        placeholder="Ej:\nCASE\n  WHEN Ventas > 10000 THEN 'Alto'\n  WHEN Ventas > 5000 THEN 'Medio'\n  ELSE 'Bajo'\nEND",
        height=200, label_visibility="collapsed", key="xl_validate_input",
    )

    col_val_btn, col_val_hint = st.columns([1, 2])
    with col_val_btn:
        run_validate = st.button("Validar fórmula", type="primary", key="xl_btn_validate")
    with col_val_hint:
    ...
    if run_validate:
        if not expr_to_validate.strip():
            ...
            with st.status("Validando fórmula Excel...", expanded=True) as status:
                st.write("Comprobando campos contra el schema...")
                st.write("Analizando sintaxis y lógica con IA...")
                try:
                    validation = validate_excel_formula(expr_to_validate, schema)
                    ss.xl_validation_result = validation
...
def _tab_explicar(schema: dict):
    ...
    if ss.xl_explain_result:
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown('<p class="card-title" style="margin-bottom:6px;">Código analizado</p>',
                    unsafe_allow_html=True)
        st.code(ss.xl_explain_result["code"], language="sql")
        st.markdown('<p class="card-title" style="margin:16px 0 8px 0;">Explicación</p>',
                    unsafe_allow_html=True)
        st.markdown(ss.xl_explain_result["explanation"])
        if st.button("Limpiar análisis", type="secondary", key="xl_clear_explain"):
            ss.xl_explain_result = None
            st.rerun()


    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<p class="card-title">Explicame esta fórmula Excel</p>', unsafe_allow_html=True)
    st.markdown(
        '<p class="hint-text">Pega cualquier campo calculado de Excel y la IA te lo explicará '
        'parte a parte: qué hace, qué funciones usa, casos de uso y alternativas.</p>',
        unsafe_allow_html=True,
    )

    expr_input = st.text_area(
        "Pega aquí tu fórmula Excel",
        placeholder="Ej:\nIF(REGEXP_MATCH(Canal, 'organic'), 'Orgánico', 'Pago')",
        height=200, label_visibility="collapsed", key="xl_explain_input",
    )

    col_exp_btn, col_exp_hint = st.columns([1, 2])
    with col_exp_btn:
        run_explain = st.button("Explicar fórmula", type="primary", key="xl_btn_explain")
    with col_exp_hint:
        st.markdown('<p class="hint-note">Funciona con cualquier fórmula Excel, aunque no sea de tu modelo actual</p>',
                    unsafe_allow_html=True)
    ...
    if run_explain:
        if not expr_input.strip():
            ...
            with st.status("Analizando la fórmula Excel...", expanded=True) as status:
                st.write("Identificando funciones y estructura...")
                st.write("Generando explicación parte a parte...")
                try:
                    explanation = explain_excel_formula(expr_input, schema)
                    ss.xl_explain_result = {"code": expr_input.strip(), "explanation": explanation}
...
def _tab_evaluar(schema: dict):
    ...
    if ss.xl_eval_result:
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown('<p class="card-title" style="margin-bottom:6px;">Fórmula evaluada</p>',
                    unsafe_allow_html=True)
        st.code(ss.xl_eval_result["code"], language="sql")
        st.markdown('<p class="card-title" style="margin:16px 0 8px 0;">Evaluación de negocio</p>',
                    unsafe_allow_html=True)
        render_resultado(ss.xl_eval_result["evaluacion"], code_lang="sql")
        if st.button("Limpiar evaluación", type="secondary", key="xl_clear_eval"):
            ss.xl_eval_result = None
            st.rerun()


    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<p class="card-title">Evaluador de Expresiones Excel</p>', unsafe_allow_html=True)
    st.markdown(
    ...
    expr_to_eval = st.text_area(
        "Pega aquí la fórmula Excel a evaluar",
        placeholder="Ej:\nCOALESCE(Ingresos, 0) / COALESCE(Visitas, 1)",
        height=200, label_visibility="collapsed", key="xl_eval_input",
    )

    col_eval_btn, col_eval_hint = st.columns([1, 2])
    with col_eval_btn:
        run_eval = st.button("Evaluar fórmula", type="primary", key="xl_btn_eval")
    with col_eval_hint:
        if schema and len(schema.get("tables", [])) > 1:
    ...
    if run_eval:
        if not expr_to_eval.strip():
            ...
            with st.status("Evaluando la lógica de negocio...", expanded=True) as status:
                ...
                try:
                    evaluacion = evaluate_excel_formula(expr_to_eval, schema)
                    ss.xl_eval_result = {"code": expr_to_eval.strip(), "evaluacion": evaluacion}
...
def _tab_chat(schema: dict):
    ...
    if ss.xl_chat_history:
        for turno in ss.xl_chat_history:
            ...
            <div class="chat-ai-bubble">
                <p class="chat-ai-label">Formula Assistant · Excel</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    pregunta_chat = st.text_area(
        ...
        height=100, label_visibility="collapsed", key="xl_chat_input",
    )

    col_chat_btn, col_chat_hint = st.columns([1, 2])
    with col_chat_btn:
        run_chat = st.button("Enviar", type="primary", key="xl_btn_chat")
    with col_chat_hint:
        st.markdown(
            f'<p class="hint-note">Conversación activa — {len(ss.xl_chat_history)} turnos</p>',
            unsafe_allow_html=True,
        )
    ...
    if run_chat:
        if not pregunta_chat.strip():
            ...
            with st.status("Consultando al modelo...", expanded=True) as status:
                ...
                try:
                    respuesta = chat_con_excel(pregunta_chat, schema, ss.xl_chat_history)
                    ss.xl_chat_history.append({
    ...
    if ss.xl_chat_history:
        if st.button("Limpiar conversación", type="secondary", key="xl_clear_chat"):
            ss.xl_chat_history = []
            st.rerun()
